...
CONFIG_DIR="${PROJECT_ROOT}/configs/llama3"
TEACHER_CONFIG="${PROJECT_ROOT}/configs/distillation/teacher_lora_llama3.yaml"
RESULTS_ROOT="${RESULTS_ROOT:-${PROJECT_ROOT}/results}"
if [[ "${RESULTS_ROOT}" != /* ]]; then
  RESULTS_ROOT="${PROJECT_ROOT}/${RESULTS_ROOT}"
fi
FAMILY_RESULTS="${RESULTS_ROOT}/llama3"
TEACHER_ADAPTER="${FAMILY_RESULTS}/teacher_lora"
RUN_ID="$(date +%Y%m%d_%H%M%S)"
LOG_DIR="${LLAMA3_LOG_DIR:-${FAMILY_RESULTS}/run_all_logs/${RUN_ID}}"
...
for override in "$@"; do
  if [[ "${override}" == resume_from_checkpoint=* ]]; then
    echo "Do not resume through train_all_llama3.sh: one checkpoint cannot be applied to every method." >&2
...
echo
echo "============================================================"
echo "Running configs/distillation/teacher_lora_llama3.yaml"
echo "Output: ${TEACHER_ADAPTER}"
echo "Log: ${LOG_DIR}/teacher_lora_llama3.log"
echo "============================================================"
...
if bash scripts/train.sh configs/distillation/teacher_lora_llama3.yaml "$@" \
  "output_dir=${TEACHER_ADAPTER}" 2>&1 | tee "${LOG_DIR}/teacher_lora_llama3.log"; then
  echo "Completed: teacher_lora_llama3"
else
  status=${PIPESTATUS[0]}
  echo "Failed: teacher_lora_llama3 (exit ${status}); dependent Llama 3 runs were not started." >&2
  exit "${status}"
fi
...
for config_name in "${CONFIG_NAMES[@]}"; do
  ...
  config_path="configs/llama3/${config_name}"
...
echo
echo "All Llama 3 configs completed successfully."
echo "Logs: ${LOG_DIR}"
