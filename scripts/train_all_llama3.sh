...
TEACHER_CONFIG="${PROJECT_ROOT}/configs/distillation/teacher_full_llama3.yaml"
...
echo
echo "============================================================"
echo "Running configs/distillation/teacher_full_llama3.yaml"
echo "Output: ${TEACHER_MODEL}"
echo "Log: ${LOG_DIR}/teacher_full_llama3.log"
echo "============================================================"
...
if bash scripts/train.sh configs/distillation/teacher_full_llama3.yaml "$@" \
  "output_dir=${TEACHER_MODEL}" 2>&1 | tee "${LOG_DIR}/teacher_full_llama3.log"; then
  echo "Completed: teacher_full_llama3"
else
  status=${PIPESTATUS[0]}
  echo "Failed: teacher_full_llama3 (exit ${status}); dependent Llama 3 runs were not started." >&2
  exit "${status}"
fi
